...
class InputHandle {
    constructor(field, container) {
        ...
        let input = createChild(extra, "input");
    ...
    constructor(fields, container) {
        this.handles = fields.map((field) => {
            switch (field.type) {
                ...
                case "input":
                    return new InputHandle(field, container);
                default:
...
class Completion extends EventTarget {
    constructor(prompt, model, options, inspector, container) {
        super();
        ...
        this.eventSource.addEventListener("message", (event) => {
            ...
            data.choices.forEach((choice) => {
                let completion = this.completions[0];
...
(async function () {
    ...
    let model = document.querySelector("#model-input");
